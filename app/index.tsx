...
import SwitchButtons from '@/components/Map/SwitchButtonMap';
import buildings from '@/assets/Buildings/BuildingsMarkers';
...
const App: React.FC = () => {
  const [region, setRegion] = useState<Region>(SGW_CAMPUS);
  ...
  const switchToSGW = () => setRegion(SGW_CAMPUS);
  const switchToLoyola = () => setRegion(LOY_CAMPUS);

  return (
    <View style={styles.container}>
      <CampusMap region={region} buildings={buildings} />
      <SwitchButtons onSwitchToSGW={switchToSGW} onSwitchToLoyola={switchToLoyola} />
    </View>
  );
};
